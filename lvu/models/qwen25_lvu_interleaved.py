...
from ..utils import post_process_kv_cache, QwenVideoReaderInterleaved
...
VIDEO_READER_BACKENDS = {
    ...
    "decord": qwen_vl_utils.vision_process._read_video_decord,
...
def chat_lvu_model(self, messages, **generation_kwargs):
    ...
    if video_group_size is not None and video_group_size > 0:
        ...
        # compensate for Qwen25_VL compressing by factor of 2 along time dimension
        vr.set_frames_per_block(video_group_size)
        pixel_iter = vr.get_pixel_iterator()

    # Use whole input when not selected
    else:
    ...
    for i, (pixel_values_videos_groups_i, pixel_block) in tqdm(enumerate(zip(pixel_values_videos_groups, pixel_iter)), 
        desc="Processing video groups", total=len(pixel_values_videos_groups), disable=not lvu_config.use_tqdm):

        assert torch.equal(pixel_values_videos_groups_i.cpu(), pixel_block), "pixels not equal"
